...
export async function POST(req) {
  try {
    ...
    if (existingBuyer) {
      return new Response(JSON.stringify({ error: "This number already exists. Please login." }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    try {
      const apiKey = "afd6c091-063e-11f0-8b17-0200cd936042"; // replace with env var
      ...
      if (response.ok && result.Status === "Success") {
        return new Response(
          JSON.stringify({ message: "OTP sent successfully", mobileNumber: formattedMobile }),
