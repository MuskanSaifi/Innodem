...
// This component contains the actual PDF structure and content.
const AgreementPDFContent = ({ data }) => {
  ...
  // Important: Add an early return if data is not available or is incomplete
  if (!data || !data.clientName) {
    // You can return a simple message or null
    return <Text>Loading agreement data...</Text>;
  }

  // A helper function to safely get the date, month, and year
  ...
  const { day, month, year } = getFormattedDate(data.createdAt);
    ...
    <Document>
      <Page style={styles.page}>
        ...
        <Text style={styles.text}>
          This Digital Services Agreement (Herein referred to as “Agreement") is made and entered into on this {day} day of {month} {year}, At Delhi.
        </Text>
        <View style={styles.section}>
          <Text style={styles.text}>Between:</Text>
          <Text style={styles.text}>INNODEM DIGITAL EXPORTS PRIVATE LIMITED THROUGH ITS DIRECTOR HAVING REGISTERED OFFICE AT: {data.serviceProviderAddress ?? 'N/A'} </Text>
          <Text style={styles.text}>AND</Text>
          <Text style={styles.text}>Client : {data.clientName ?? 'N/A'}</Text>
          <Text style={styles.text}>[Company Address]: {data.companyAddress ?? 'N/A'}</Text>
          <Text style={styles.text}>[Email Address]: {data.email ?? 'N/A'}</Text>
          <Text style={styles.text}>[Phone Number]: {data.phone ?? 'N/A'} (Hereinafter referred to as the “Client”)</Text>
        </View>
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.text}>{data.serviceDetails ?? 'N/A'}</Text>
          <Text style={styles.text}>
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.text}>2.1 This Agreement shall commence on {data.startDate ?? 'N/A'} and shall continue until {data.endDate ?? 'N/A'} or until terminated by either party as outlined herein.</Text>
          <Text style={styles.text}>2.2 Either party may terminate this Agreement with thirty (30) days' written notice however it is hereby clarified that once the payment is received by the service provider from the client i.e., {data.clientName ?? 'N/A'} the same shall not returned in any event after signing of the present agreement as all the conditions of the said scheme taken by the client is vernacularly narrated to the client.</Text>
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.text}>3.3 That by signing of the present agreement it has been undertaken by the client that the payment received by the service provider is legitimate one and no misrepresentation was made to get the payment dated: {data.paymentDate ?? 'N/A'} by the service provider from the client.</Text>
          ...
          <Text style={styles.text}>3.5 No. of days after signing of Agreement, the service/ membership amount will be refunded back after deducting (1)01-07Days- 30% (2) 08-15 Days- 50% (3)16-21 Days- 75% (4) 21 Days - 100%.</Text>
        </View>
        ...
        <View style={styles.signatureSection}>
          ...
          <View style={{ width: '45%' }}>
            ...
            <Text style={styles.signatureText}>{data.clientName ?? 'N/A'}</Text>
          </View>
        </View>
      </Page>
    </Document>
...
const PDFClientSideWrapper = ({ userAgreement, handleDownloadComplete }) => {
  // Ensure userAgreement is not null or undefined
  if (!userAgreement) {
    return <p>No agreement data available.</p>;
  }

  const agreementDocument = <AgreementPDFContent data={userAgreement} />;

  return (
    <div className="flex flex-col items-center w-full h-full">
      <PDFViewer
        className="w-full h-[600px] my-4 border border-gray-300 rounded-lg"
        style={{ width: '100%', height: '600px' }}
      >
        {agreementDocument}
      </PDFViewer>

{userAgreement.status === 'signed' && (
  <PDFDownloadLink
    document={agreementDocument}
    fileName={`service-agreement-${userAgreement._id?.substring(0, 5)}.pdf`}
    onClick={() => handleDownloadComplete(userAgreement._id)}
  >
    {({ loading }) => (
      <button
        className="mt-4 bg-blue-600 text-white px-6 py-3 rounded-full shadow-lg hover:bg-blue-700 transition-all duration-300 font-semibold"
        disabled={loading}
      >
        {loading ? 'Generating PDF...' : 'Download PDF with Signature'}
      </button>
    )}
  </PDFDownloadLink>
)}
...
export default PDFClientSideWrapper;